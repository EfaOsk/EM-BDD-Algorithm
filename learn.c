        ...
        for (int t = 0; t < T; t++) {
            for (int o = 0; o < M; o++) {
                ...
                else if (o < M-1)
                {
                    ...
                    Cudd_Ref(AO[u][t][o]);
                    for (int j = 0; j < o; j++){
                        DdNode *temp = Cudd_bddOr(manager, AO[u][t][o], Cudd_Not(AO_enc[u][t][j]));
                        Cudd_Ref(temp);
                        Cudd_RecursiveDeref(manager, AO[u][t][o]);
                        AO[u][t][o] = temp;
                    }
                    DdNode *temp = Cudd_bddAnd(manager, AO[u][t][o], AO_enc[u][t][o]);
                    Cudd_Ref(temp);
                    Cudd_RecursiveDeref(manager, AO[u][t][o]);
                    AO[u][t][o] = temp;
        ...
        for (int t = 0; t < T-1; t++) {
            for (int v = 0; v < N; v++) {
                ...
                else if (v < N-1)
                {
                    ...
                    Cudd_Ref(AS[u][t][v]);
                    for (int j = 0; j < v; j++){
                        DdNode *temp = Cudd_bddOr(manager, AS[u][t][v], Cudd_Not(AS_enc[u][t][j]));
                        Cudd_Ref(temp);
