...
int main(int argc, char *argv[]) {
    ...
    sprintf(experiment_name, "experiments_0_BDD");
    ...
    for (int m = 0; m < 11; m++) {
        ...
        for (int obs = 0; obs < 10; obs++) {
            for (int i = 0; i < 10; i++) {
                ...
                FILE *result_fp = fopen("experiments/experiments_0_BDD/results.txt", "a");
                ...
                HMM *learned_model = learn(hypothesis_hmm, T, obs_seq[obs], 0.0001, result_folder, "experiments/experiments_0_BW/results.txt");
            }
        }
    }
