...
int main(int argc, char *argv[]) {
    ...
    sprintf(experiment_name, "experiments_1_BDD");
    ...
    fprintf(metadata, "Hypothesis models: random_models\n");
    ...
    for (int m = 0; m < 11; m++) {
        ...
        for (int obs = 0; obs < 10; obs++) {
            int *obs_seq = HMM_generate_sequence(example_models[m], T);
            for (int i = 0; i < 10; i++) {
                ...
                FILE *result_fp = fopen("experiments/experiments_1_BDD/results.txt", "a");
                fprintf(result_fp, "Model: %s, Obs: %d, Run: %d, L(Org): %f, ", example_models[m]->name, obs, i, log_likelihood_forward(example_models[m], obs_seq, T));
                fclose(result_fp);
                HMM* hypothesis_hmm = HMM_random_create(example_models[m]->N, example_models[m]->M, "hypothesis model");
                HMM *learned_model = learn(hypothesis_hmm, T, obs_seq, 0.001, result_folder, "experiments/experiments_1_BDD/results.txt");
            }
        }
    }
